...
static void histogram__scale_map(double *map, size_t size, uint8_t min,
                                 uint8_t max, uint8_t *out) {
  double mmin = map[0], mmax = map[size - 1];
  for (unsigned i = 0; i < size; i++)
    out[i] = (map[i] - mmin) * (max - min) / (mmax - mmin) + min;
}

static void histogram__frame(filter *filt, const y4m2_parameters *parms,
                             y4m2_frame *frame) {
  uint32_t hist[256];
  double fwd_map[256];
  double rev_map[256];
  ...
  for (unsigned p = 0; p < Y4M2_N_PLANE; p++) {
    if (model_get_int(&filt->config, 0, "$.options.%s.disabled",
      ...
      histogram__scale_map(rev_map, 256, 16, p == Y4M2_Y_PLANE ? 235 : 240,
                           xmap);
...
static void histogram__end(filter *filt) { y4m2_emit_end(filt->out); }

void histogram_register(void) {
  filter f = {.start = histogram__start,
